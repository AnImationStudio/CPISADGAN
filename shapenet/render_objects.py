...
output_basepath = "../../data/shapenet/"

object_type = "airplane" # "trash_bin" # 
num_objects = 55
obj_num = 2
width, height, focal = 320, 240, np.pi/6 #640, 480, np.pi/3

obj_id_list, obj_name_list, name_to_id = get_object_to_name(BASE_PATH)
name_to_filelist = get_all_filelist(BASE_PATH, obj_id_list)

view_path = './rendering_meta.txt'
...
for index, param in enumerate(cam_params):
	# print("The output index ", index, param)
	for angle in [+1, -1]:
		out_idx = int(2*index+(angle+1)/2)
		param[1] = param[1]*angle
